...
class MetricsLogger:
    ...
    def log_metrics(self, metrics: Dict[str, float], step: int, phase: str = "train"):
        ...
        # Local logging
        self.metrics_history.append({"step": step, "phase": phase, **metrics})
...
class AutonomousDrivingDataset(Dataset):
    ...
    def __getitem__(self, idx: int) -> dict:
        ...
        multi_camera_tensor = torch.stack(
            camera_images, dim=0
        )  # [num_cameras, 3, H, W]
...
class E2ETrainer:
    ...
    def compute_loss(
        ...
        bev_occupancy: torch.Tensor | None = None,
    ) -> LossComponents:
        ...
        if (
            "camera_features" in intermediates
            and len(intermediates["camera_features"]) > 1
        ):
            features = torch.stack(
                intermediates["camera_features"], dim=1
            )  # [B, num_cameras, feature_dim]
            # Compute pairwise consistency
            feature_diff = features.unsqueeze(2) - features.unsqueeze(
                1
            )  # [B, num_cameras, num_cameras, feature_dim]
    ...
    def adversarial_training_step(self, batch: Dict[str, torch.Tensor]) -> torch.Tensor:
    ...
    def train_epoch(self, train_loader: DataLoader) -> Dict[str, float]:
        ...
        for batch_idx, batch in enumerate(train_loader):
            ...
            if batch_idx % 50 == 0:
                self.logger.info(
                    f"Epoch {self.current_epoch}, Batch {batch_idx}/{num_batches}, Loss: {total_loss.item():.4f}"
                )
        ...
        with torch.no_grad():
            for batch in val_loader:
                ...
                predictions, intermediates = self.model(
                    multi_camera_input, camera_poses
                )
    ...
    def train(self, train_loader: DataLoader, val_loader: DataLoader):
        ...
        for epoch in range(self.config.num_epochs):
            ...
            self.metrics_logger.log_metrics(
                {"learning_rate": current_lr}, self.global_step, "train"
            )
