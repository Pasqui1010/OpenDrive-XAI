...
class ExpertModel(nn.Module):
    ...
    def __init__(
        ...
        self.confidence_head = nn.Sequential(
            nn.Linear(feature_dim, 64), nn.ReLU(), nn.Linear(64, 1), nn.Sigmoid()
        )

    def forward(
        self, image_sequence: torch.Tensor, return_confidence: bool = True
...
class MBRAProcessor:
    ...
    def process_passive_video(self, data_source: PassiveDataSource) -> Dict:
        ...
        logger.info(
            f"Processing {data_source.source_type} video: {data_source.video_path}"
        )
...
def create_youtube_dataset_config() -> ReAnnotationConfig:
    ...
    return ReAnnotationConfig(expert_horizon=15, min_video_length=100, target_fps=8)
