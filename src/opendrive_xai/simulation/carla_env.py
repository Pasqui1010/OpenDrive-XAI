    ...
    def connect(self) -> bool:
        ...
        try:
            ...
            if self.map_name not in available_maps:
                self.logger.warning(
                    f"Map {self.map_name} not found, using first available"
                )
        ...
        try:
            ...
            for config in camera_configs:
                ...
                camera_transform = carla.Transform(
                    carla.Location(
                        x=config.location[0], y=config.location[1], z=config.location[2]
                    ),
                    carla.Rotation(
                ...
                camera.listen(
                    lambda image, sensor_id=config.sensor_id: self._on_camera_data(
                        weak_self, image, sensor_id
                    )
                )
        ...
        try:
            ...
            for sensor_id in self.cameras.keys():
                if sensor_id in self.sensor_data:
                    camera_images[sensor_id] = self.sensor_data[sensor_id][
                        "image"
                    ].copy()
    ...
    def save_episode(self, data: List[SensorData], output_dir: Path):
        ...
        with open(poses_file, "w") as f_poses, open(metadata_file, "w") as f_meta:
            for i, frame in enumerate(data):
                ...
                f_poses.write(
                    f"{timestamp} {loc[0]:.6f} {loc[1]:.6f} {loc[2]:.6f} "
                    f"{qx:.6f} {qy:.6f} {qz:.6f} {qw:.6f}\n"
                )

                # Save metadata
                f_meta.write(
                    f"{timestamp}: frame_id={frame.frame_id}, "
...
def create_default_camera_setup() -> List[CameraConfig]:
    ...
    return [
        ...
        CameraConfig(
            sensor_id="front_left", location=(1.8, -0.5, 1.4), rotation=(0, -45, 0)
        ),
        CameraConfig(
            sensor_id="front_right", location=(1.8, 0.5, 1.4), rotation=(0, 45, 0)
        ),
        CameraConfig(sensor_id="rear", location=(-2.0, 0.0, 1.4), rotation=(0, 180, 0)),
        CameraConfig(
            sensor_id="rear_left", location=(-1.8, -0.5, 1.4), rotation=(0, -135, 0)
        ),
        CameraConfig(
            sensor_id="rear_right", location=(-1.8, 0.5, 1.4), rotation=(0, 135, 0)
        ),
    ]
