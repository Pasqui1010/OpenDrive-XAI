...
"""
    ...
    @staticmethod
    def create_safety_verification_prompt(rules: List[ASPRule], context: Dict[str, Any]) -> str:
        """Create prompt for LLM to verify safety of generated rules."""
        
        rules_text = "\n".join([f"- {rule.rule_text}: {rule.natural_language}" for rule in rules])
...
class LLMInterface:
    ...
    async def generate_asp_rules(self, 
                               instruction: str, 
                               context: Dict[str, Any]) -> List[ASPRule]:
        """
        ...
        prompt = self.prompt_template.create_rule_generation_prompt(instruction, context)
    ...
    async def _verify_rule_safety(self, 
                                rules: List[ASPRule], 
                                context: Dict[str, Any]) -> List[ASPRule]:
        ...
        safety_prompt = self.prompt_template.create_safety_verification_prompt(rules, context)
    ...
    def _apply_safety_conditions(self, 
                               rules: List[ASPRule], 
                               safety_response: Dict[str, Any]) -> List[ASPRule]:
        ...
        for rule in rules:
            ...
            if "safe_conditions" not in rule.rule_text:
                ...
                rule.confidence *= 0.9  # Slight confidence reduction for added conditions
